                    ...
                    <table class="table mb-0">
                        ...
                        @foreach($pendingRequests as $suggestionRequest)
                            <tr>
                                ...
                                <td><a href="{{ route('admin.suggestion-requests.edit', $suggestionRequest->id) }}">{{ $suggestionRequest['status'] }}</a></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function updateTable(status) {
                ...
                tableBody.innerHTML = requests.map(request => `
                    <tr>
                        <th scope="row"><a href="/admin/suggestion-requests/edit/${request.id}">${request.id}</a></th>
                        <td><a href="/admin/suggestion-requests/edit/${request.id}">${request.name}</a></td>
                        <td><a href="/admin/suggestion-requests/edit/${request.id}">${request.phone}</a></td>
                        <td><a href="/admin/suggestion-requests/edit/${request.id}">${request.message}</a></td>
